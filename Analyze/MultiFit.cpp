...
void FitterThread::SetReplica( int idx_, bool bShowOutput, Matrix *pCorrel, Vector * pStdErrorMean )
{
  ...
  if( idx_ != idx )
  {
    ...
    if( !parent.bFreezeCovar || bFirstTime )
    {
      ...
      else
      {
        ...
        if( pCorrel )
        {
          assert( pCorrel->size1 == parent.ds.Extent && pCorrel->size2 == parent.ds.Extent
                 && "Debug save correlation matrix in HDF5" );
          ( *pCorrel ) = Cholesky;
          pCorrel->CholeskyScaleApply( CholeskyDiag );
          for( int i = 1; i < pCorrel->size1; ++i )
            for( int j = 0; j < i; ++j )
            {
              ( *pCorrel )( i, j ) = Cholesky( j, i );
              ( *pCorrel )( j, i ) = Cholesky( j, i );
            }
          for( int i = 0; i < pCorrel->size1; ++i )
            ( *pCorrel )( i, i ) = 1.;
        }
#ifdef DUMP_MATRICES
        {
      ...
      if( pStdErrorMean )
      {
        assert( pStdErrorMean->size == parent.ds.Extent && "Debug save error vector in HDF5" );
        for( int i = 0; i < parent.ds.Extent; ++i )
          ( *pStdErrorMean )[i] = 1. / CholeskyDiag[i];
      }
    }
                    ...
                    const std::string &ModelSuffix, Common::SeedType Seed, const Common::CovarParamsRebin &cpr )
{
  ...
  if( bCorrelated )
    OutputModel.Correl.resize( ds.Extent, ds.Extent );
  OutputModel.StdErrorMean.resize( ds.Extent );
  ...
  if( bPerformFit )
  {
        ...
        try
        {
...
#endif
          {
            ...
            if( bCorrelated )
            {
              ...
              try
              {
                fitThread.SetReplica( Fold::idxCentral, true, &OutputModel.Correl, &OutputModel.StdErrorMean );
          ...
          for( int idx = Fold::idxCentral; idx < ds.NSamples; ++idx )
          {
            try
            {
              ...
              if( !WasAbort )
              {
                if( idx == Fold::idxCentral )
                {
                  fitThread.SetReplica( idx, false, &OutputModel.Correl, &OutputModel.StdErrorMean );
                  ChiSq = fitThread.FitOne( parGuess );
                }
                else
                {
                  fitThread.SetReplica( idx, false, nullptr, nullptr );
                          fitThread.FitOne( parGuess );
                }
              }
            }
            catch( const std::exception &e )
    ...
    // Save the correlation matrix to file if requested
    if( bCorrelated && bSaveCMat )
    {
      if( model.size() != ds.corr.size() )
        throw std::runtime_error( "ModelSet doesn't match DataSet" );
      std::vector<std::vector<std::string>> ExtraStrings( ds.corr.size() );
      for( std::size_t f = 0; f < model.size(); ++f )
      {
        ...
        for( std::size_t op = 1; op < model[f]->ParamNamesPerExp.size(); ++op )
          ExtraStrings[f][0].append( model[f]->ParamNamesPerExp[op] );
        if( model[f]->ParamNamesPerExp.size() == 2 )
          ExtraStrings[f][0].append( model[f]->ParamNamesPerExp[1] );
        // Operators - one-off
        for( const std::string &op : model[f]->ParamNames )
        {
        ...
        for( const std::string &op : model[f]->ParamNamesPerExp )
        {
      ...
      ds.SaveCovariance( Common::MakeFilename( sModelBase, Common::sCormat, Seed, TEXT_EXT ),
                         OutputModel.Correl, &ExtraStrings );
    }
    // Save the file
